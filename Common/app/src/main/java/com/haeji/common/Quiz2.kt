...
class Quiz2 : AppCompatActivity(), View.OnClickListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mQuestionList = Constants2.getQuestions() //전체프로젝트범위에 속하지못함 그래서 위에설정
                ...
                if (mSelectedOptionPosition == 0) { //선택한 옵션위치가 0인 상황에서는 다음위치로 이동
                    ...
                    when{
                        mCurrentPosition <= mQuestionList!!.size -> {
                            ...
                            val intent = Intent(this, ResultActivity2::class.java) //이름값을 result에 넘겨줌
                            intent.putExtra(Constants2.USER_NAME, mUserName)//액티비티간 값을 넘겨줄때 사용
                            intent.putExtra("quiz2", mCorrectAnswers)
                            intent.putExtra(Constants2.TOTAL_QUESTIONS, mQuestionList!!.size)
